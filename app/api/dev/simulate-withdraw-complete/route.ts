...
export async function POST(request: NextRequest) {
  ...
  if (record.transaction.status === TransactionStatus.FAILED) {
    return NextResponse.json({ message: "Cannot complete a failed withdrawal." }, { status: 409 });
  }

  if (record.transaction.status !== TransactionStatus.PROCESSING) {
    return NextResponse.json({ message: "Withdrawal must be in PROCESSING to complete." }, { status: 409 });
  }
